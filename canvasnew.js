...
window.initCanvasAnimation = function () {
  var canvas = document.getElementById("animated-canvas");
  if (!canvas) {
  ...
  var count = 0;
  var colorPalette = [
    [0x00 / 255, 0x16 / 255, 0x36 / 255],
    [0x0b / 255, 0x49 / 255, 0xa2 / 255],
    [0x48 / 255, 0xb0 / 255, 0xf7 / 255],
    [0x69 / 255, 0x4d / 255, 0xe8 / 255],
  ];

  var colorArr = colorPalette[Math.floor(Math.random() * colorPalette.length)];

  canvas.addEventListener("mousemove", function (e) {
    count++;

    if (count > 25) {
      colorArr = colorPalette[Math.floor(Math.random() * colorPalette.length)];
      count = 0;
    }
